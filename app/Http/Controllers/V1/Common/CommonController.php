        ...
        if (null != $license) {
            try {
                ...
                if (count($settings) > 0) {
                    ...
                    $appsettings['referral']                  = (1 == $settings->site->referral) ? 1 : 0;

                    $appsettings['social_login'] = (1 == $settings->site->social_login) ? 1 : 0;
                    $appsettings['send_sms']     = (1 == $settings->site->send_sms) ? 1 : 0;
                    $appsettings['send_email']   = (1 == $settings->site->send_email) ? 1 : 0;
                    $appsettings['otp_verify']   = (1 == $settings->transport->ride_otp) ? 1 : 0;

                    $appsettings['ride_otp'] = (1 == $settings->transport->ride_otp) ? 1 : 0;

                    $appsettings['order_otp']   = (1 == $settings->order->order_otp) ? 1 : 0;
                    $appsettings['date_format'] = (isset($settings->site->date_format)) ? $settings->site->date_format : 0;

                    $appsettings['service_otp'] = (1 == $settings->service->serve_otp) ? 1 : 0;
