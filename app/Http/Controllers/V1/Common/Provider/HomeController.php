...
class HomeController extends Controller
{
    ...
    public function acceptedList(Request $request)
    {
        ...
        try {
            ...
            $response = (new ProviderServices())->acceptedList($request);
    ...
    public function rejectedList(Request $request)
    {
        ...
        try {
            ...
            $response = (new ProviderServices())->rejectedList($request);
    ...
    public function show_profile()
    {
        ...
        if ($provider_details) {
            $provider_details->employee_since = Carbon::parse($provider_details->created_at)->diffForHumans();
        }
    ...
    public function adminservices(Request $request)
    {
        ...
        foreach ($adminservices_list as $key => $adminservice) {
            ...
            $category_ids     = '';
            $sub_category_ids = '';
            if ('SERVICE' == $adminservice->admin_service) {
            ...
            $data      = new \stdClass;
            $documents = Document::with(['provider_document', 'serviceCategory', 'servicesubCategory'])->where('company_id', Auth::guard('provider')->user()->company_id)->where('type', $adminservice->admin_service)->where('status', 1)->when('SERVICE' == $adminservice->admin_service, function ($q) use ($category_ids, $sub_category_ids, $type) {
    ...
    public function addvechile(Request $request)
    {
        ...
        try {
            ...
            if ('TRANSPORT' == $request->admin_service || 'ORDER' == $request->admin_service || 'DELIVERY' == $request->admin_service) {
                $providerservice->save();
            }
    ...
    public function services_status(Request $request, $id)
    {
        if (!empty($id)) {
            ...
            if ("ASSESSING" == $provider_service->status) {
            ...
            if ('SERVICE' == $request->admin_service) {
                ...
                if ($request->has("service")) {
                    foreach ($request->service as $key => $val) {
                        ...
                        if (!empty($existingservice) && "ASSESSING" == $existingservice->status) {
                            $status = "ASSESSING";
                        } else {
    ...
    {
        $banks = Bank::where('active', 'YES')->get();
        return Helper::getResponse(['data' => $banks]);
    }
    ...
    public function addbankdetails(Request $request)
    {
        ...
        $this->validate($request,
            [
                'bankform_id.*'                  => 'required',
                'keyvalue.*.account_number'      => 'required',
                'keyvalue.*.account_holder_name' => 'required',
                'keyvalue.*.ifsc'                => 'required',
                'keyvalue.*.bank_name'           => 'required',
                'keyvalue.*.bank_id'             => 'required',
            ]);
        ...
        try {
            ...
            for ($i = 0; $i < count($request->bankform_id); $i++) {
                ...
                $providerbank->keyvalue    = json_encode($request->keyvalue[$i], true);
    ...
    public function onlinestatus(Request $request, $id)
    {
        try {
            ...
            $provider = Provider::findOrFail(Auth::guard('provider')->user()->id);
            if ('APPROVED' == $provider['status'] && 1 == $provider['activation_status']) {
