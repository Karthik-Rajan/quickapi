...
class ProviderAuthController extends Controller
{
	...
	public function listdocuments(Request $request) {
        ...
        $sub_category_ids = ProviderService::where('admin_service',$type)->where('provider_id',Auth::guard('provider')->user()->id)->get()->pluck('sub_category_id');
  		...
  		$documents = Document::with(['provider_document','serviceCategory','servicesubCategory'])->where('company_id', Auth::guard('provider')->user()->company_id)->where('type',$type)->where('status',1)->when($type == 'SERVICE', function ($q) use( $category_ids,$sub_category_ids,$type) {
	...
	public function document_store(Request $request) {
		...
		try {
            ...
            $document_list=Document::WhereIn('type',$document)->where('status',1)->where('company_id',Auth::guard('provider')->user()->company_id)->when($servicetype == 'SERVICE', function ($q) use( $category_ids,$sub_category_ids,$servicetype) {
					if(!empty($category_ids))
					{
