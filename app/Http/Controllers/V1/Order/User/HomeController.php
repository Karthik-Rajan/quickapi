...
class HomeController extends Controller
{
    ...
    public function store_list(Request $request, $id)
    {
        ...
        if ($request->has('qfilter') && '' != $request->qfilter) {
            if ('non-veg' == $request->qfilter) {
                $store_list_all->where('is_veg', 'Non Veg');
            }
            if ('pure-veg' == $request->qfilter) {
                $store_list_all->where('is_veg', 'Pure Veg');
            }
            if ('freedelivery' == $request->qfilter) {
    ...
    public function all_products(Request $request)
    {
        ...
        $all_products = StoreItem::with('store', 'brand', 'attribute', 'attribute_value')->where('status', 1)->where('company_id', $company_id)->get();
    ...
    public function show_addons(Request $request, $id)
    {
        ...
        if ("PERCENTAGE" == $item_addons->item_discount_type) {
    ...
    public function addcart(Request $request)
    {
        ...
        if ("PERCENTAGE" == $Item->item_discount_type) {
    ...
    public function findCart($request, $checkAddons)
    {
        ...
        foreach ($cartIds as $cartId) {
            ...
            if (0 == $addonCheck && count($checkAddons) == 0) {
                $cart = $cartId;
                break;
            }

            if (count($checkAddons) == $addonCheck) {
                foreach ($checkAddons as $checkAddon) {
                    ...
                    if (count($checkAddons) == $addonStatus) {
                        $status = false;
                        break;
                    }
                }
    ...
    public function viewcart(Request $request)
    {
        ...
        try {
            ...
            if (!$CartItems->isEmpty()) {
                ...
                if (!empty($request->user_address_id)) {
                    ...
                    if ($address_details) {
                        ...
                        if ('Kms' == $this->settings->site->unit_measurement) {
                            $distance = round($distance / 1000, 1);
                        }
                        //TKM
                        else {
                            $distance = round($distance / 1609.344, 1);
                        }
                        //TMi
                    }
                    if ($cityprice) {
                        if (1 == $Product->store->free_delivery && 'DELIVERY' == $request->order_type) {
                            ...
                            if ('TAKEAWAY' == $request->order_type) {
                                $free_delivery = 0;
                            } else {
                                $free_delivery = $cityprice->delivery_charge + (($distance - $cityprice->base_distance) * $cityprice->distance_price);
                    ...
                    $free_delivery = ('TAKEAWAY' == $request->order_type) ? 0 : null;
                }
                ...
                if ($request->wallet && "" != $request->wallet && 1 == $request->wallet) {
            ...
            if ($request->has('checkout_status') && 1 == $request->checkout_status) {
                return $Cart;
            }
    ...
    public function orderDeliveryTime($id)
    {
        try {
            ...
            if ('STARTED' == $store_order->status || 'PICKEDUP' == $store_order->status || 'REACHED' == $store_order->status) {
    ...
    public function checkout(Request $request)
    {
        ...
        if ($address_details && 'DELIVERY' == $request->order_type) {
        ...
        if ($order->id) {
            ...
            if ('DELIVERY' == $request->order_type) {
            ...
            if ($orderinvoice->wallet_amount == $orderinvoice->total_amount && 0 == $orderinvoice->payable) {
            ...
            if ("OTHERS" == $store_details->storetype->category) {
            ...
            if ($request->has('delivery_date') && '' != $request->delivery_date) {
                ...
                $requestData = ['type' => 'ORDER', 'room' => 'room_' . $this->company_id, 'id' => $order->id, 'city' => (0 == $this->settings->demo_mode) ? $order->store->city_id : 0, 'user' => $order->user_id];
    ...
    public function status(Request $request)
    {
        try {
            if ($request->input('status')) {
                $check_status = explode(',', $request->input('status'));
                $isInclude    = true;
            } else {
            ...
            $orderRequest = StoreOrder::OrderRequestStatusCheck($userId, $check_status, $admin_service->id, $isInclude)
                ->get()
                ->toArray();
    ...
    public function getOrderHistorydetails(Request $request, $id)
    {
        try {
            ...
            $userrequest = StoreOrder::with(["store.storetype", 'orderInvoice' => function ($query) {
    ...
    public function requestStatementHistory(Request $request)
    {
        try {
            ...
            if ($request->has('status') && null != $request->status) {
                $history_status = [$request->status];
            }

            if ($request->has('user_id') && null != $request->user_id) {
                $orderRequests->where('user_id', $request->user_id);
            }

            if ($request->has('provider_id') && null != $request->provider_id) {
                $orderRequests->where('provider_id', $request->provider_id);
            }

            if ($request->has('ride_type') && null != $request->ride_type) {
    ...
    public function order_request_dispute(Request $request)
    {
        ...
        if (null == $order_request_disputes) {
            try {
    ...
    public function get_order_request_dispute(Request $request, $id)
    {
        ...
        if ($order_request_dispute) {
            $order_request_dispute->created_time = (\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $order_request_dispute->created_at, 'Asia/Kolkata'))->setTimezone($order_request_dispute->request->timezone)->format(Helper::dateFormat());
        }
