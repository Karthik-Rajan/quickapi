...
class StoreOrder extends BaseModel
{
    ...
    public function scopeHistoryUserTrips($query, $user_id, $showType = '')
    {
        ...
        if ('' != $showType) {
            ...
            } else if ('upcoming' == $showType) {
                $history_status = ['SCHEDULED'];
            } else {
                $history_status = ['ORDERED', 'RECEIVED', 'STORECANCELLED', 'SEARCHING', 'ACCEPTED', 'STARTED', 'ARRIVED', 'PICKEDUP', 'REACHED', 'DROPPED'];
            }
    ...
    public function scopeOrderRequestStatusCheck($query, $user_id, $check_status, $admin_service, $isInclude = false)
    {
        if ($user_id) {
            $query = $query->where('store_orders.user_id', $user_id);
        }

        if ($isInclude) {
            $query = $query->whereIn('store_orders.status', $check_status);
        } else {
            $query = $query->whereNotIn('store_orders.status', $check_status);
        }
        return $query->where('store_orders.user_rated', 0)
