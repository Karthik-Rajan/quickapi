...
class RideRequest extends BaseModel
{
    ...
    public function scopeHistoryUserTrips($query, $user_id, $showType = '')
    {
        if ('' != $showType) {
            if ('past' == $showType) {
                ...
                $history_status = ['SEARCHING', 'ACCEPTED', 'STARTED', 'ARRIVED', 'PICKEDUP', 'DROPPED'];
            }
    ...
    public function getTotalDistanceAttribute()
    {
        ...
        if ('KMS' == $this->unit) {
            $total_distance = round($this->distance / 1000, 1);
        }
        //TKM
        else {
            $total_distance = round($this->distance / 1609.344, 1);
        }
        //TMi
    ...
    {
        return (isset($this->attributes['created_at'])) ? (\Carbon\Carbon::createFromFormat('Y-m-d H:i:s', $this->attributes['created_at'], 'Asia/Kolkata'))->setTimezone($this->attributes['timezone'])->format(Helper::dateFormat(1)) : '';
    }
