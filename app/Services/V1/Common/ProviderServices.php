...
class ProviderServices
{
    ...
    public function provider_cancel($IncomingRequests, $settings)
    {
        ...
        for ($i = 0; $i < count($IncomingRequests); $i++) {
            ...
            //Transport
            if ("TRANSPORT" == $admin_service) {
                $config = $settings->transport;
                $is_otp = $config->ride_otp;
            }

            //Service
            elseif ("SERVICE" == $admin_service) {
        ...
        try {
            ...
            try {
                if ("TRANSPORT" == $request->admin_service) {
                    ...
                    if (null != $this->user->admin_id) {
                        $newRequest->admin_id = $this->user->admin_id;
                    }

                    if ("SEARCHING" != $newRequest->status) {
            ...
            try {
                if ("SERVICE" == $request->admin_service) {
                    $validator = Validator::make($request->all(), [
                    ...
                    if ("SEARCHING" != $newRequest->status && "SCHEDULED" != $newRequest->status) {
            ...
            try {
                if ("DELIVERY" == $request->admin_service) {
                    $validator = Validator::make($request->all(), [
                    ...
                    try {
                        ...
                        $newRequest = \App\Models\Delivery\DeliveryRequest::with('delivery')->where('id', $request->id)->first();
            ...
            if ("" != $newRequest->schedule_at) {
                ...
                try {
                    ...
                    if ("SERVICE" == $request->admin_service) {
                        ...
                        \App\Models\Service\ServiceRequest::
                            where('provider_id', $this->user->id)
                            ->where('booking_id', $newRequest->booking_id)
                ...
                if ("SERVICE" == $request->admin_service) {
                    ...
                    //Send message to socket
                    $requestData = ['type' => 'DELIVERY', 'room' => 'room_' . $this->company_id, 'id' => $newRequest->id, 'city' => (0 == $this->settings->demo_mode) ? $newRequest->city_id : 0, 'user' => $newRequest->user_id, 'message' => 'testing'];
                } else {
                    $newRequest->status = "STARTED";
                    $newRequest->save();
                    //Send message to socket
                    $requestData = ['type' => 'TRANSPORT', 'room' => 'room_' . $this->company_id, 'id' => $newRequest->id, 'city' => (0 == $this->settings->demo_mode) ? $newRequest->city_id : 0, 'user' => $newRequest->user_id, 'message' => 'testing'];
                }
                ...
                if ("DELIVERY" == $newRequest->admin_service) {
                    $publishUrl = 'checkDeliveryRequest';
                }
    ...
    {
        if ("TRANSPORT" == $request->admin_service) {
            $acceptedList = \App\Models\Transport\RideRequest::where('status', 'STARTED')
                ->where('provider_id', $this->user->id)
                ->get();
        }

        if ("ORDER" == $request->admin_service) {
            $acceptedList = \App\Models\Order\StoreOrder::where('status', 'PROCESSING')
                ->where('provider_id', $this->user->id)
                ->get();
        }

        if ("DELIVERY" == $request->admin_service) {
            $acceptedList = \App\Models\Delivery\DeliveryRequest::where('status', 'STARTED')
                ->where('provider_id', $this->user->id)
                ->get();
        }

        if ("SERVICE" == $request->admin_service) {
            $acceptedList = \App\Models\Service\ServiceRequest::where('status', 'ACCEPTED')
                ->where('provider_id', $this->user->id)
                ->get();
        }

        return ['status' => 200, 'message' => '', 'data' => $acceptedList];
    }

    public function rejectedList(Request $request)
    {
        if ("TRANSPORT" == $request->admin_service) {
            $cancelledList = \App\Models\Transport\RideRequest::where('status', 'CANCELLED')
                ->where('provider_id', $this->user->id)
                ->get();
        }

        if ("ORDER" == $request->admin_service) {
            $cancelledList = \App\Models\Order\StoreOrder::where('status', 'CANCELLED')
                ->where('provider_id', $this->user->id)
                ->get();
        }

        if ("DELIVERY" == $request->admin_service) {
            $cancelledList = \App\Models\Delivery\DeliveryRequest::where('status', 'CANCELLED')
                ->where('provider_id', $this->user->id)
                ->get();
        }

        if ("SERVICE" == $request->admin_service) {
            $cancelledList = \App\Models\Service\ServiceRequest::where('status', 'CANCELLED')
                ->where('provider_id', $this->user->id)
                ->get();
        }

        return ['status' => 200, 'message' => '', 'data' => $cancelledList];
    }
    ...
    public function cancelRequest(Request $request)
    {
        ...
        if ("ORDER" == $request->admin_service) {
            ...
            if ("TRANSPORT" == $request->admin_service) {
                ...
                if (1 != $config->broadcast_request || 'AIRPORT' == $ride_request->type) {
                    $this->assignNextProvider($request->id, $request->admin_service);
                }
            } else {
                if (1 != $config->broadcast_request) {
                    $this->assignNextProvider($request->id, $request->admin_service);
                }
            }
    ...
    {
        if ('' != $config && 0 == $config->manual_request) {
            ...
            if ('SEARCHING' == $IncomingRequest->request->status && $IncomingRequest->request->time_left_to_respond < 0) {
                if ("TRANSPORT" == $IncomingRequest->request->request->admin_service && 'AIRPORT' == $IncomingRequest->request->request->type) {
        ...
        if (null != $userRequestLast) {
            if ("TRANSPORT" == $admin_service) {
                ...
                if (count($Providers) > 0) {
                    ...
                    //Send message to socket
                    $requestData = ['type' => $admin_service, 'room' => 'room_' . $this->company_id, 'id' => $id, 'city' => (0 == $this->settings->demo_mode) ? $newRequest->city_id : 0, 'user' => $userRequestLast->user_id];
                    app('redis')->publish('newRequest', json_encode($requestData));
                }
            }
        }
    ...
    public function cancel($id, $admin_service)
    {
        if ("TRANSPORT" == $admin_service) {
            try {
                ...
                $requestData = ['type' => 'TRANSPORT', 'room' => 'room_' . $this->company_id, 'id' => $newRequest->id, 'user' => $newRequest->user_id, 'city' => (0 == $this->settings->demo_mode) ? $newRequest->city_id : 0];
                app('redis')->publish('newRequest', json_encode($requestData));

                $requestData = ['type' => 'TRANSPORT', 'room' => 'room_' . $this->company_id, 'id' => $newRequest->id, 'user' => $newRequest->user_id, 'city' => (0 == $this->settings->demo_mode) ? $newRequest->city_id : 0];
                app('redis')->publish('checkTransportRequest', json_encode($requestData));
            } catch (\Throwable $e) {}
        } else if ("ORDER" == $admin_service) {
            try {
                ...
                $requestData = ['type' => 'ORDER', 'room' => 'room_' . $this->company_id, 'id' => $newRequest->id, 'user' => $newRequest->user_id, 'city' => (0 == $this->settings->demo_mode) ? $newRequest->city_id : 0];
                app('redis')->publish('newRequest', json_encode($requestData));
            ...
            $requestData = ['type' => 'DELIVERY', 'room' => 'room_' . $this->company_id, 'id' => $newRequest->id, 'user' => $newRequest->user_id, 'city' => (0 == $this->settings->demo_mode) ? $newRequest->country_id : 0];
            app('redis')->publish('newRequest', json_encode($requestData));
    ...
    public function assignNextProvider($request_id, $admin_service)
    {
        ...
        if ("TRANSPORT" == $admin_service) {
            try {
                ...
                $requestData = ['type' => 'TRANSPORT', 'room' => 'room_' . $this->company_id, 'id' => $newRequest->id, 'user' => $newRequest->user_id, 'city' => (0 == $setting->demo_mode) ? $newRequest->city_id : 0];
            } catch (\Throwable $e) {}
        } else if ("ORDER" == $admin_service) {
            try {
                $newRequest = \App\Models\Order\StoreOrder::with('user')->find($UserRequest->request_id);

                $requestData = ['type' => 'ORDER', 'room' => 'room_' . $this->company_id, 'id' => $newRequest->id, 'user' => $newRequest->user_id, 'city' => (0 == $setting->demo_mode) ? $newRequest->city_id : 0];
            } catch (\Throwable $e) {}
        } else if ("SERVICE" == $admin_service) {
            try {
                $newRequest = \App\Models\Service\ServiceRequest::with('user')->find($UserRequest->request_id);

                $requestData = ['type' => 'SERVICE', 'room' => 'room_' . $this->company_id, 'id' => $newRequest->id, 'user' => $newRequest->user_id, 'city' => (0 == $setting->demo_mode) ? $newRequest->city_id : 0];
            } catch (\Throwable $e) {}
        } else if ("DELIVERY" == $admin_service) {
            //try {
            ...
            $requestData = ['type' => 'DELIVERY', 'room' => 'room_' . $this->company_id, 'id' => $newRequest->id, 'user' => $newRequest->user_id, 'city' => (0 == $setting->demo_mode) ? $newRequest->country_id : 0];
