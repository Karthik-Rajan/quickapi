...
class Ride
{
    ...
    public function createRide(Request $request)
    {
        ...
        $child_seat  = null != $request->child_seat ? $request->child_seat : 0;
        $wheel_chair = null != $request->wheel_chair ? $request->wheel_chair : 0;
        ...
        try {
            ...
            if ("1" == $this->settings->transport->manual_request) {
            ...
            // dd($request->someone);
            if ($request->has('someone') && 1 == $request->someone) {
                ...
                try {
                    if (!empty($request->someone_email) && !empty($this->settings->site->send_email) && 1 == $this->settings->site->send_email && $this->settings->transport->ride_otp) {
                    ...
                    if (!empty($request->someone_mobile) && !empty($this->settings->site->send_sms) && 1 == $this->settings->site->send_sms && $this->settings->transport->ride_otp) {
            ...
            if (null == $request->d_latitude && null == $request->d_longitude) {
            ...
            if ($request->has('schedule_date') && "" != $request->schedule_date && $request->has('schedule_time') && "" != $request->schedule_time) {
                ...
                $rideRequest->schedule_at  = (Carbon::createFromFormat('Y-m-d H:i:s', (Carbon::parse($request->schedule_date . ' ' . $request->schedule_time)->format('Y-m-d H:i:s')), $timezone))->setTimezone('Asia/Kolkata');
            ...
            return ['message' => ('SCHEDULED' == $rideRequest->status) ? 'Schedule request created!' : 'New request created!', 'data' => [
                'message' => ('SCHEDULED' == $rideRequest->status) ? 'Schedule request created!' : 'New request created!',
    ...
    public function cancelRide($request)
    {
        ...
        try {
            ...
            if ('CANCELLED' == $rideRequest->status) {
            ...
            if (in_array($rideRequest->status, ['SEARCHING', 'STARTED', 'ARRIVED', 'SCHEDULED'])) {
                if ('SEARCHING' != $rideRequest->status) {
    ...
    public function extendTrip(Request $request)
    {
        try {
            ...
            $requestData = ['type' => 'TRANSPORT', 'room' => 'room_' . $this->company_id, 'id' => $rideRequest->id, 'city' => (0 == $this->settings->demo_mode) ? $rideRequest->city_id : 0, 'user' => $rideRequest->user_id];
    ...
    public function updateRide(Request $request)
    {
        ...
        if ('DROPPED' == $request->status && null != $request->d_latitude && null != $request->d_longitude) {
            $rideRequest->d_latitude  = $request->d_latitude;
            $rideRequest->d_longitude = $request->d_longitude;
        ...
        if ('DROPPED' == $request->status && 'CASH' != $rideRequest->payment_mode) {
            $rideRequest->status = 'COMPLETED';
            $rideRequest->paid   = 0;

            (new SendPushNotification)->Complete($rideRequest, 'transport');
        } else if ('COMPLETED' == $request->status && 'CASH' == $rideRequest->payment_mode) {
            if ('COMPLETED' == $rideRequest->status) {
            ...
            if (1 != $RequestPayment->is_partial && $RequestPayment->wallet > 0) {
        ...
        if ('PICKEDUP' == $request->status) {
            ...
            if (null != $provider->airport_at) {
                Provider::where('id', $provider->id)->update(['airport_at' => null]);
            }

            if (1 == $this->settings->transport->ride_otp && "ADMIN" != $rideRequest->created_type) {
                if (isset($request->otp) && "MANUAL" != $rideRequest->request_type) {
        ...
        if ('DROPPED' == $request->status) {
            ...
            if ($request->has('distance') && null != $rideRequest->distance) {
                $rideRequest->distance = $request->distance;
            }

            if ($request->has('location_points') && null != $rideRequest->location_points) {
            ...
            $invoice              = $this->invoice($request->id, (null != $request->toll_price) ? $request->toll_price : 0);
    ...
    public function calculateFare($request, $cflag = 0)
    {
        ...
        try {
            ...
            if (!empty($location['errors'])) {
                ...
                if ('Kms' == $siteConfig->unit_measurement) {
    ...
    public function applyPriceLogic($requestarr, $iflag = 0)
    {
        ...
        $per_minute    = (null == $ride_city_price) ? 0 : $ride_city_price->minute; //PM
        $per_hour      = (null == $ride_city_price) ? 0 : $ride_city_price->hour; //PH
        $per_kilometer = (null == $ride_city_price) ? 0 : $ride_city_price->price; //PKM
        $base_distance = (null == $ride_city_price) ? 0 : $ride_city_price->distance; //BD
        $base_price    = (null == $ride_city_price) ? 0 : $ride_city_price->fixed; //BP
        $price         = 0;
        if (null != $ride_city_price) {
            if ('MIN' == $ride_city_price->calculator) {
                //BP+(TM*PM)
                $price = $base_price + ($total_minutes * $per_minute);
            } else if ('HOUR' == $ride_city_price->calculator) {
                //BP+(TH*PH)
                $price = $base_price + ($total_hours * $per_hour);
            } else if ('DISTANCE' == $ride_city_price->calculator) {
                //BP+((TKM-BD)*PKM)
            ...
            } else if ('DISTANCEMIN' == $ride_city_price->calculator) {
                //BP+((TKM-BD)*PKM)+(TM*PM)
                if ($base_distance > $total_kilometer) {
                    $price = $base_price + ($total_minutes * $per_minute);
                } else {
            ...
            } else if ('DISTANCEHOUR' == $ride_city_price->calculator) {
                //BP+((TKM-BD)*PKM)+(TH*PH)
                if ($base_distance > $total_kilometer) {
                    $price = $base_price + ($total_hours * $per_hour);
                } else {
        ...
        $fn_response['calculator']      = (null == $ride_city_price) ? null : $ride_city_price->calculator;
    ...
    public function getLocationDistance($locationarr)
    {
        ...
        try {
            ...
            if (!empty($location['rows'][0]['elements'][0]['status']) && 'ZERO_RESULTS' == $location['rows'][0]['elements'][0]['status']) {
    ...
    public function invoice($request_id, $toll_price = 0)
    {
        ...
        if ('Kms' == $siteConfig->unit_measurement) {
            $total_distance = round($rideRequest->distance / 1000, 1);
        }
        //TKM
        else {
            $total_distance = round($rideRequest->distance / 1609.344, 1);
        }
        //TMi
        ...
        if (1 == $rideRequest->use_wallet && $payable_amount > 0) {
            ...
            if (0 != $Wallet) {
                if ($payable_amount > $Wallet) {
                    ...
                    if ('CASH' == $rideRequest->payment_mode) {
