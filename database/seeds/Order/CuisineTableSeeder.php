...
class CuisineTableSeeder extends Seeder
{
    ...
    public function run($company = null)
    {
        ...
        $Medicine = DB::connection('order')->table('store_types')->where('name', 'Medicine')->first()->id;
        $Products = DB::connection('order')->table('store_types')->where('name', 'Products')->first()->id;
